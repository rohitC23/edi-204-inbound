...
def convert_to_docx(doc_file_path):
    try:
        ...
        document.LoadFromFile(doc_file_path)

        # Save the DOC file to DOCX format
        docx_file_path = os.path.splitext(doc_file_path)[0] + ".docx"
        document.SaveToFile(docx_file_path, FileFormat.Docx2016)
...
def upload_file():
    try:
        ...
        # Convert to DOCX
        docx_file_path = convert_to_docx(doc_file_path)

        return jsonify({"uploaded_filename": os.path.basename(docx_file_path)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
